...
class BookViewSetTestCase(TestCase):
    def setUp(self):
        self.book1 = Book.objects.create(title='Book 1', author='Author 1', cover='HARD', inventory=10, daily_fee=10.00)
        self.book2 = Book.objects.create(title='Book 2', author='Author 2', cover='SOFT', inventory=5, daily_fee=8.00)
    ...
    def test_non_admin_cannot_modify_books(self):
        ...
        response = self.client.put(f'/api/books/{self.book1.id}/', {'title': 'Updated Title'})
